...
# ── Port configuration ───────────────────────────────────────────────────────
# Overridable via environment variables so a second worktree / WeekenD instance
# can run alongside other services without clashing. Defaults preserve the
# original behaviour (8001 / 3000 / 2026).

GATEWAY_PORT="${DEERFLOW_GATEWAY_PORT:-8001}"
FRONTEND_PORT="${DEERFLOW_FRONTEND_PORT:-3000}"
NGINX_PORT="${DEERFLOW_NGINX_PORT:-2026}"

# Resolved nginx config (generated from the template via envsubst at runtime so
# the port placeholders get substituted). Lives under logs/ to stay repo-local.
NGINX_TEMPLATE="$REPO_ROOT/docker/nginx/nginx.local.conf"
NGINX_RUNTIME_CONF="$REPO_ROOT/logs/nginx.runtime.conf"
...
_is_repo_nginx_pid() {
    ...
    case "$args" in
        *"$REPO_ROOT/docker/nginx/nginx.local.conf"*|*"$NGINX_RUNTIME_CONF"*|*"$REPO_ROOT"*) return 0 ;;
    esac
...
stop_all() {
    ...
    nginx -c "$NGINX_RUNTIME_CONF" -p "$REPO_ROOT" -s quit 2>/dev/null || true
    sleep 1
    _kill_repo_nginx
...
# 3. Nginx
# Generate the runtime nginx config from the template, substituting only the
# three port placeholders so the rest of the template is left untouched.
DEERFLOW_GATEWAY_PORT="$GATEWAY_PORT" \
DEERFLOW_FRONTEND_PORT="$FRONTEND_PORT" \
DEERFLOW_NGINX_PORT="$NGINX_PORT" \
    envsubst '${DEERFLOW_GATEWAY_PORT} ${DEERFLOW_FRONTEND_PORT} ${DEERFLOW_NGINX_PORT}' \
    < "$NGINX_TEMPLATE" > "$NGINX_RUNTIME_CONF"

run_service "Nginx" \
    "nginx -g 'daemon off;' -c '$NGINX_RUNTIME_CONF' -p '$REPO_ROOT' > logs/nginx.log 2>&1" \
    "$NGINX_PORT" 10
